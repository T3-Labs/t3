...
def _create_basic_project(project_path: Path, name: str) -> None:
    ...
    (project_path / "src" / "main.py").write_text(f'"""Main module for {name}."""\n\ndef main():\n    print("Hello from {name}!")\n\nif __name__ == "__main__":\n    main()\n')
...
@init_app.command()
def docker(
    force: bool = typer.Option(False, "--force", "-f", help="Force pull image even if exists"),
    ...
    try:
        ...
        console.print(f"⚠️ Warning: Failed to pull Docker image: {e.stderr}", style="yellow")
        console.print("💡 Continuing with configuration setup...", style="blue")
    except FileNotFoundError:
        console.print("⚠️ Warning: Docker not found. Continuing with configuration setup...", style="yellow")
    ...
    try:
        ...
        console.print(f"✅ Configuration file created: {config_file_path.absolute()}", style="green")
        ...
        console.print(f"   docker run -d --name t3-edge-video \\")
        console.print(f"     -p 8080:8080 -p 3000:3000 -p 1935:1935 \\")
        console.print(f"     -v $(pwd)/data:/app/data \\")
        console.print(f"     -v $(pwd)/config:/app/config \\")
        console.print(f"     -v $(pwd)/logs:/app/logs \\")
        console.print(f"     --env-file <(grep -v '^#' config.yaml | grep '=' || true) \\")
        console.print(f"     ghcr.io/t3-labs/edge-video:latest")
        ...
        try:
            ...
            console.print("❌ Failed to install PyYAML. Please install manually: pip install pyyaml", style="red")
            raise typer.Exit(1)
